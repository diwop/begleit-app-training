...
- **Base Image:** We chose an Axolotl image to leverage an official Axolotl environment where flash-attention and deepspeed are already pre-compiled and configured for PyTorch and CUDA 13.0.
...
2. It checks the `MODE` environment variable. 
   ...
   - If `MODE=eval`, it runs `bash scripts/eval.sh` to isolate the `src-eval` dependencies and execute the SGLang batch evaluation.
