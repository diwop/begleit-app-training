        ...
        if base_model_str and base_model_str not in processed_models:
            ...
            try:
                ...
                snapshot_download(
                    ...
                    token=token,
                    ignore_patterns=["*.msgpack", "*.h5", "*.ot", "*.pt"] # Strip alternate formats to save disk space
                )
...
def run_training_job(config_path: str, num_gpus: int, run_id: str):
    """
    Isolates configuration merging, dynamic deepspeed building,
    ...
    # -------------------------------------------------------------------------
    # AUTOMATED ARCHITECTURE ROUTING LAYER
    # -------------------------------------------------------------------------
...
def main():
    ...
    # FORCE COOLDOWN: Deactivate the experimental vLLM V1 graph compiler for child evaluation tasks
